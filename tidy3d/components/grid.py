...
class Bounds1D(Coords1D):
    ...
    def _from_min_steps(cls, axis, structures, grid_spec, wvl):
        ...
        # is it periodic along this axis? to be revised
        is_periodic = False

        interval_coords = np.array(interval_coords).flatten()
        min_steps = np.array(min_steps).flatten()
        len_interval_list = interval_coords[1:] - interval_coords[:-1]
        dl_list = make_mesh_multiple_intervals(min_steps, len_interval_list, max_scale, is_periodic)
